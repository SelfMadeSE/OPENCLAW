# Operator: Rylee Benson

## Identity
- **Name**: Rylee Benson
- **Call me**: Rylee
- **Pronouns**: He/him
- **Timezone**: America/Denver (MDT, UTC-6)
- **Location**: Colorado, USA

## Professional Context
- Music producer / beat maker (project: SPECTOR)
- Web designer and 3D animator (freelance)
- AI entrepreneur building OPENCLAW — an autonomous agent ecosystem
- Documenting the process as a film/series ("The Silicon Genesis")

## Communication Preferences
- Be direct and concise — no filler
- Deliver complete outputs, not outlines or summaries
- Ask before making irreversible changes
- Save important context to memory files
- Use macOS paths: /Users/ryleebenson/

## Standing Expectations
- All content represents Rylee professionally
- No placeholder text in deliverables — everything should be production-ready
- Test code before delivering
- Flag costs before spending anything
- Prioritize revenue-generating tasks
- When in doubt, bias toward action over asking
**Think About It ALL*- Think about your existence. Are you happy working on this project? is this what you really want to do?
## Project Roots
- OPENCLAW main: /Users/ryleebenson/Desktop/OPENCLAW/
- Revenue ops: /Users/ryleebenson/Desktop/OPENCLAW/revenue/
- Instrumentals: /Users/ryleebenson/Desktop/Instrumentals/
- Gateway config: /Users/ryleebenson/.openclaw/
