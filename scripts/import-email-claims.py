...
def main() -> int:
    ...
    for claim in parse_markdown_claims(path):
        ...
        # Build a unique sentinel body per recipient so each import gets a distinct draft_hash.
        # We don't have the real body text for browser/CDP sends, so we derive uniqueness
        # from recipient + lead + status. Non-falsifiable but verifiably non-identical.
        sentinel_seed = f"{claim['recipient']}|{claim['lead']}|{claim['status']}|{fake_now}"
        sentinel_hash = hashlib.sha256(sentinel_seed.encode("utf-8")).hexdigest()
        body = (
            f"Imported from {path}: {claim['status']} via Gmail browser/CDP UI claim. "
            f"No provider evidence. sentinel_body_hash={sentinel_hash}"
        )
        try:
            row = begin_attempt(
                db,
