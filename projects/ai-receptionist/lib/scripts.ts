/**
 * Dental AI Receptionist — Call Scripts (TypeScript)
 *
 * All scripts are designed to sound warm, calm, and reassuring.
 * Dental patients are often nervous — every word should ease that,
 * not add to it.
 */
...
export function appointmentConfirm(
  ...
  return (
    `Perfect! I've got you requested for ${day} in the ${time} for a ${reason}. You'll receive a text shortly with the details. Our team will confirm availability and get back to you.\n\n` +
...
export function voicemailPrompt(): string {
  return "Please leave your name, phone number, and a brief message after the tone. This message may be recorded or transcribed. We'll return your call on the next business day.";
}

// ─── SMS Templates ───────────────────────────────────────────────────

export function smsToCaller(
  businessName: string,
  day: string,
  time: string,
  reason: string
  ...
  return (
    `Hi! Your appointment at ${businessName} has been requested for ${day} at ${time} (${reason}). Our team will confirm shortly. ` +
    "We look forward to seeing you! Reply HELP for assistance, STOP to opt out."
  );
}

export function smsToOwner(
  businessName: string,
  name: string,
  day: string,
  time: string,
  ...
  return (
    `📲 New appointment at ${businessName}: ${name} on ${day} at ${time} for ${reason}. ` +
    `Caller: ${phone}. Demo session: ${sessionId}`
  );
}

export function smsEmergencyInfo(
  businessName: string,
  emergencyNumber: string
): string {
  return (
    `${businessName} Emergency Line: ${emergencyNumber}. ` +
    "If you're in urgent pain, don't wait — call now or visit your nearest ER. Reply STOP to opt out."
  );
}
